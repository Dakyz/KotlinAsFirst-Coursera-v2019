...
class Tests {
    ...
    @Tag("Impossible")
    fun russian() {
        assertEquals("семьсот сорок семь тысяч семьсот", russian(747700))
        assertEquals("восемьсот тридцать девять тысяч сто девятнадцать", russian(839119))
        assertEquals("восемьсот семьдесят четыре тысячи восемьсот пятьдесят", russian(874850))
        assertEquals("двенадцать", russian(12))
        assertEquals("двадцать две тысячи девятьсот шестьдесят четыре", russian(22964))
        assertEquals("две тысячи сто сорок один", russian(2141))
